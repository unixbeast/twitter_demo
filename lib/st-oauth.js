// todo - normalize/create object for token
...
STOAuthClient.prototype.getAuthUrl = function(action) {
    // todo - passing additional query parameters to redirect_uri BREAKS! WHY!?
    action = action || "";
    ...
    console.log('generated auth url: ' + uri);
    return uri;
};
...
STOAuthClient.prototype.getAccessToken = function(code, callback){
    console.log("In STOAuth getAccessToken");
    var tokenConfig = {
    ...
    this._request(tokenConfig, function(error, response, body) {
        if (error || response.statusCode != 200) {
            // todo better error handling, might be null
            ...
            console.log('token expires_at: ' + tokenInfo.expires_at);
            ...
            that._request(endpointConfig, function(error, response, body) {
                if (error) {
                    console.log('got error getting endpoint: ' + error);
                    ...
                    // TODO - why are there so many entries with different endpoints?
                    console.log('apiJson length: ' + apiJson.length);
...
STOAuthClient.prototype.post = function(opts, callback) {
    ...
    if (opts.headers) {
        console.log("HEADERS EXIST YO, WILL ADD AUTH HEADER");
        opts.headers['Authorization'] = 'Bearer ' + opts.token;
    } else {
        console.log("HEADERS DID NOT EXIST, WILL CREATE");
        opts.headers = {'Authorization': 'Bearer ' + opts.token};
    }
...
STOAuthClient.prototype._request = function(opts, callback) {
    ...
    console.log('will make request with options: ' + JSON.stringify(requestConfig));

    request(requestConfig, function(error, response, body) {
        // todo - handle non-200 response codes
        console.log('in request handler, error: ' + JSON.stringify(error));
        console.log('in request handler, response: ' + JSON.stringify(response));
        console.log('in request handler, body: ' + JSON.stringify(body));

        var testStatus = JSON.parse(response);
        console.log(testStatus.statusCode);
        var status = response.statusCode;
        console.log('in request handler, statusCode: ' + status);
        ...
        if (error || !(status >= 200 || status < 300 || status === 304)) {
            ...
            console.log('no error on request, statusCode: ' + response.statusCode);
            callback(null, response, body);
        }
    });
};
